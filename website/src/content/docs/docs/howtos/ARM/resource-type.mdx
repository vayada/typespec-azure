...
Unlike the standard envelope property spreads below, `BillingDataProperty` is spread into the
resource-specific properties model. `BillingData` is available in ARM common-types v6, so select
`Azure.ResourceManager.CommonTypes.Versions.v6` with `@armCommonTypesVersion` before using it.
