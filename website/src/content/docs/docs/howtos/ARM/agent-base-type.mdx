...
`ConversationProperties` and `ResponseProperties` both require an `input` property of type
`InputItem`. `InputItem` represents a message, function call, function-call output, or compaction
item. Its `type` property selects the variant; the other fields are optional because only the
fields for the selected variant are populated. Message `content` and function `arguments` use
`Record<unknown>` so the input can carry structured content. Response output uses the separate,
read-only `ResponseItem` model and can be added with `ResponseOutputProperty`.
...
multiple times to declare conformance to multiple base types; duplicate entries are ignored. The
`Agent` template is preferred because it supplies the current base type and version automatically:

```typespec
#suppress "@azure-tools/typespec-azure-resource-manager/basetypes-experimental" "Experimental BaseTypes"
@azureBaseType(#{ baseType: BaseType.Agent, version: "2026-04-01" })
model ContosoPlatformAgent is TrackedResource<ContosoPlatformAgentProperties> {
