...
### Limitation: avoiding type conflicts with templated parameters

When `@clientLocation` is applied to a **templated** parameter (for example, a common type or alias used by multiple operations that each instantiate it with a different type), the parameters share the same name but have different types. Moving them all to the same client causes a type collision.

In this situation, TCGC emits a `client-location-conflict` warning:

> `@clientLocation cannot move multiple parameters named '<name>' with different types to the same client. This often happens when @clientLocation is applied to a templated parameter that is instantiated with different types. Move the parameter on each operation instead, so that it has a consistent type on the client.`

To resolve this warning, apply `@clientLocation` on each individual operation parameter rather than on the shared templated parameter.
