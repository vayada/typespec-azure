...
### Avoiding parameter type conflicts with templated operations

When `@clientLocation` is used to move a parameter from an operation to a client, and the same parameter name resolves to **different types** across multiple template instantiations, the compiler emits a `client-location-conflict` warning.

This commonly occurs when `@clientLocation` is applied to a templated parameter that is instantiated with different types. For example:

```tsp
// ❌ This causes a client-location-conflict warning:
// The 'resource' parameter has type `FooResource` in one instantiation
// and `BarResource` in another, but both are moved to the same client.
@@clientLocation(FooOperations.createFoo::parameters.resource, MyClient);
@@clientLocation(BarOperations.createBar::parameters.resource, MyClient);
```

The recommended fix is to **not use `@clientLocation` for these parameters** and instead keep each parameter at the operation level:

```tsp
// ✅ Keep per-operation parameters at the operation level.
// Remove the conflicting @clientLocation augmentations and let each
// operation carry its own typed parameter.
```

If you need the parameter at the client level, ensure all moved instances share the exact same type. Otherwise, leave the parameter on each operation individually.
