...
Rule documentation lives beside each rule implementation in
`packages/typespec-azure-resource-manager/src/rules/*.md`. Each `createRule` declaration associates
its page through `docs: fileRef.fromPackageRoot(...)`. The old
`website/src/content/docs/docs/libraries/azure-resource-manager/rules/` copies were removed when the
repository migrated to `fileRef`-backed rule docs. `reference/linter.md` is generated and links to
the published rule routes.

## Reference Documentation Generation

- `pnpm regen-docs` from `packages/typespec-azure-resource-manager/` generates the tracked
  `reference/` files (data-types.md, decorators.md, interfaces.md, index.mdx, linter.md).
- The same command generates ignored website rule pages under
  `website/src/content/docs/docs/libraries/azure-resource-manager/rules/` from `src/rules/*.md`.
  These generated pages are needed for local website builds but are not tracked.
...
The former `arm-resource-operation` linter rule was split into three focused rules:
`use-api-version`, `use-interface`, and `use-operation-decorator`. The old rule is no longer
registered.
...
`BillingDataProperty` is a deliberate exception: spread it into the resource-specific properties
bag, as shown by `resource-common-properties/billing-data/main.tsp` and its source doc comment.
`BillingData` is available starting with ARM common-types v6.
...
- The Agent base type version is `2026-04-01`.
- Child property bases: `ConversationProperties`, `ResponseProperties`; mix-ins
  `PreviousResponseProperty`, `ResponseOutputProperty`, and `ResponseInstructionsProperty`.
  `ConversationProperties.input` and `ResponseProperties.input` are required `InputItem` values.
  `InputItem.content`, `InputItem.arguments`, `ResponseItem.content`, and
  `ResponseItem.arguments` use `Record<unknown>`.
...
- New linting rules (registered in `src/linter.ts`, with docs under `src/rules/`):
  `arm-agent-base-type-child-resources` (Agent must have both a Conversation and a Response child),
  `arm-agent-base-type-lifecycle-operations` (those children need full CRUD),
  `no-reserved-resource-property`, `arm-custom-resource-usage-discourage`, and
...
- `Relationship<Properties>` is an `ExtensionResource` template in
  `Azure.ResourceManager.BaseTypes.Relationships`. It applies
  `@azureBaseType(#{ baseType: BaseType.Relationship, version: "2026-04-01" })` automatically.
- `RelationshipProperties<ProvisioningState = ResourceProvisioningState>` provides read-only
  `baseTypes` and `provisioningState`, plus required `sourceId`, `sourceTenant`, `targetId`, and
  `targetTenant`.
- The `use-relationship-required-properties` rule verifies that Relationship resources are
  extension resources with every required property.
- Canonical sample:
