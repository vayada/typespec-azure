...
## Streaming & SSE Metadata (July 2026)

- `SdkStreamMetadata` (pre-existing) is attached via `streamMetadata?` on `SdkBodyParameter` and on HTTP responses when the body is a streaming type (`JsonlStream`, `SSEStream`). Fields: `bodyType`, `originalType` (stream model), `streamType` (payload), `contentTypes`.
- NEW: `SdkSseMetadata` / `SdkSseEventMetadata` and the `sseMetadata?` property were added to body param and both response interfaces. Populated only for `text/event-stream` (SSE) streams whose streamed type is an `@events` union; `undefined` for JSONL. Built by `buildSdkSseMetadata` in `src/http.ts` using `@typespec/sse` (`isEvents`, `isTerminalEvent`) and `@typespec/events` (`getEventDefinitions`).
- `SdkSseEventMetadata` fields: `eventType` (SSE `event:` name; undefined => `message` event), `isTerminalEvent`, `isEventEnvelope` (envelope wraps a separate `@data` payload), `type`/`contentType` (envelope), `payloadType`/`payloadContentType`.
- Neither streaming nor SSE was documented in guideline.md before. Added a "Streaming and Server-Sent Events" subsection after "HTTP Operation Response Calculation". Emitter-developer-facing; no Spector spec needed (type-graph metadata; covered by `test/methods/sse.test.ts` and `test/methods/streams.test.ts`).

## versionsEnum on SdkClientType (July 2026)

- `SdkClientType.versionsEnum?: SdkEnumType` added — the API-versions enum for the client's single service. `undefined` for unversioned services and multi-service (merged) clients (`client.services.length !== 1`). Resolved via `context.getPackageVersionSdkEnum()` (Map<Namespace, SdkEnumType>), backed by internal `__serviceToVersionsSdkEnum`. Documented in guideline.md Client section.

## @encode(string) on boolean (July 2026)

- `addEncodeInfo` in `src/types.ts` now allows `@encode(string)` on `boolean` (previously integer kinds only). Result: `SdkBuiltInType` kind `boolean` with `encode: "string"`. Added a one-line note to the integer-encoded-as-string prose in 08types.mdx (no new ClientTabs block).

## Linter rule churn (July 2026)

- `no-unnamed-types` rule was added then REMOVED within this window (net: gone from `src/rules/` and `src/linter.ts`). Don't document it.
- Active rules: `require-client-suffix`, `property-name-conflict`, `csharp-no-url-suffix` (last two also in `csharpRules`). Their `.md` `docs`-field files were enriched (impact/severity/message/suppression) by their source PRs — already in tree.
- `reference/linter.md` is auto-generated from the rule `docs` fields and already reflects the 3-rule set; no manual edit needed. The `../rules/<name>.md` page links it emits are generated by the website build, not committed under the docs tree.
