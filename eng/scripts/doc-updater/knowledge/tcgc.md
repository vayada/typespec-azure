...
## Incremental Update (July 2026) — serializationOptions on collection types

- `SdkArrayType` and `SdkDictionaryType` now expose an optional `serializationOptions?: SerializationOptions` (added in `src/interfaces.ts`). It is populated ONLY when the collection is a named model with explicit serialization decorators — e.g. `@Xml.name("Foo") model Foo is Bar[];` or `@encodedName("application/xml", "Xml...") model Foo is Bar[];`. Anonymous inline arrays/dicts leave it `undefined`; their wrapping element name comes from the referencing property/model. Documented in guideline.md Collection Types bullet.
- Implementation: `setSerializationOptions`/`updateJsonSerializationOptions`/`updateXmlSerializationOptions` in `src/types.ts` now also accept `SdkArrayType | SdkDictionaryType`, and `type.serializationOptions` is lazily created (`??= {}`) instead of assumed present. The array/dict branch calls `setSerializationOptions(context, type, [])` with an empty content-type list so only explicitly-defined info is captured. Regression tests: `test/types/serialization-options.test.ts` (`#3978`).
- `@clientLocation` + `@client` cross-scope fix (#4850): the client-name/location validator now skips an operation belonging to an explicit `@client` scoped to a different language (via `isClientForOtherScopeOnly` in `src/validations/types.ts`), so a `@clientLocation` for other scopes plus a `@client(..., "java")` re-declaration no longer falsely report a duplicate operation. Internal validation fix — no user doc change needed.
- `SdkTestLibrary` (from `src/testing/index.ts`) is now `@deprecated` in favor of `createTester` from `@typespec/compiler/testing`. Not referenced in any doc page, so no doc change required.
