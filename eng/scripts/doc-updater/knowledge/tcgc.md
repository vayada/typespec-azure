...
- `override-parameters-mismatch` (error): Pre-existing diagnostic (fires when a required original parameter has no match in the override). As of July 2026 it also fires when the override **drops `@path`** from a parameter that is a _realized_ path parameter of the original operation's HTTP route. "Realized" matters: a `@path` in the type graph is not enough (e.g. an ARM key carried by a templated provider action is not part of the route). `@override` now matches parameters **by name** (not sorted position), so overrides may reorder/add params. The `@path` check is skipped when any override parameter carries `@clientLocation`. Source: `$override` in `src/decorators.ts` (`getRealizedPathParamNames` resolves the route via `getHttpOperation`). Documented in 04method.mdx under "Parameter matching rules".
- `client-location-conflict` (warning): Gained a new `parameterTypeConflict` message ID (July 2026). Fires when `@clientLocation` moves the same-named parameter to the same client (namespace/interface) with **different types** — typically a templated parameter (e.g. on an `alias`) instantiated with different types across operations. Fix: apply `@clientLocation` per operation so the client parameter has one consistent type. Validation lives in `validateClientLocationParameterTypes` in `src/validations/types.ts`. Documented in 04method.mdx under "Keeping parameter types consistent when moving to the client".

## Linter Rules

Rules live in `packages/typespec-client-generator-core/src/rules/` and are registered in `src/linter.ts`. Each rule has a manual doc page in `website/.../typespec-client-generator-core/rules/<name>.md` and is listed in the auto-generated `reference/linter.md` (produced by `pnpm regen-docs`).

- Rules as of July 2026: `require-client-suffix`, `property-name-conflict`, `no-unnamed-types`, `csharp-no-url-suffix`.
- RuleSets: `all` (all rules) and `best-practices:csharp` (csharp-scoped rules). `property-name-conflict` and `csharp-no-url-suffix` are in both.
- `csharp-no-url-suffix` (warning): Flags model properties whose **C#-resolved** name (via `getLibraryName(..., "csharp")`, respecting `@clientName`) ends with `Url`; suggests replacing the trailing `l` with `i` (→ `Uri`). Case-sensitive (`url`/`Urls` are not flagged). Ships a codefix that appends `@@clientName("<name>Uri", "csharp")` to `client.tsp` (creating it and adding imports/usings if needed). The codefix helpers are generic and reusable: `createAugmentDecoratorCodeFix` and `createClientTspAugmentDecoratorCodeFix` in `src/rules/codefix-helpers.ts`. Already fully documented — no doc action needed when only this rule changes.
- Adding a rule requires: the rule page under `rules/`, and regenerating `reference/linter.md`. Do NOT hand-edit `reference/linter.md`.
